...
function shell(body: string, footer = 'Terima kasih telah berbelanja di Jualakun.id.'): string {
  ...
  <div style="font-weight:700;font-size:20px;color:#111">Jualakun<span style="color:#06B6D4">.id</span></div>
  <div style="margin-top:16px;line-height:1.6">${body}</div>
  <hr style="border:none;border-top:1px solid #E5E7EB;margin:24px 0"/>
  <div style="font-size:12px;color:#6B7280">${footer} · <a href="${SITE}" style="color:#06B6D4">jualakun.id</a></div>
</div>`
}
  ...
  orderCreated: (p: { fullName: string; orderNumber: string; productName: string; totalIdr: number; snapUrl: string }) => ({
    ...
    emailHtml: shell(
      `<p>Halo <strong>${p.fullName}</strong>,</p>
       ...
       <p><a href="${p.snapUrl}" style="display:inline-block;background:#06B6D4;color:#fff;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:600">Lanjutkan Pembayaran</a></p>
  ...
  accountDelivered: (p: { fullName: string; orderNumber: string; productName: string; credentials: string; note: string | null; guaranteeExpiresAt: string }) => ({
    ...
    emailHtml: shell(
      `<p>Halo <strong>${p.fullName}</strong>,</p>
       ...
       <p><a href="${SITE}/dashboard" style="color:#06B6D4">Lihat di dashboard →</a></p>`,
    ),
  }),
  ...
  ticketReplaced: (p: { fullName: string; orderNumber: string; resolution: string }) => ({
    ...
    emailHtml: shell(
      `<p>Halo <strong>${p.fullName}</strong>,</p>
       ...
       <blockquote style="border-left:3px solid #06B6D4;padding:8px 12px;margin:16px 0;color:#374151">${p.resolution}</blockquote>
       <p>Akun pengganti sudah dapat diakses di <a href="${SITE}/dashboard" style="color:#06B6D4">dashboard</a>.</p>`,
    ),
  }),
  ...
  referralCredited: (p: { fullName: string; creditAmount: number; totalCredits: number }) => ({
    ...
    emailHtml: shell(
      `<p>Halo <strong>${p.fullName}</strong>,</p>
       ...
       <p><a href="${SITE}/dashboard/referral" style="color:#06B6D4">Lihat detail referral →</a></p>`,
    ),
  }),
