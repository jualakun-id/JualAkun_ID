import Link from 'next/link'
import { Mail, Phone, Clock, ShieldCheck, AlertCircle } from 'lucide-react'
import { Logo } from '@/components/branding/logo'

export function Footer() {
  return (
    <footer className="bg-[#1F2937] text-white">
      <div className="container mx-auto px-4 py-14 max-w-7xl">
        {/* ── TOP: Brand intro ─────────────────────────────── */}
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8 pb-10 border-b border-white/10">
          <div className="max-w-sm w-full flex flex-col items-center md:items-start text-center md:text-left">
            <Logo size="lg" showTagline inverted />
            <p className="mt-5 text-[15px] text-white/80 leading-relaxed">
              Marketplace akun digital langka & sulit dicari di tempat lain — premium, asli, dengan garansi resmi.
            </p>
          </div>
          <div className="flex items-center gap-2 self-center md:self-end text-sm text-white/75 font-medium">
            <ShieldCheck size={16} className="text-brand-400" />
            <span>Terdaftar &amp; Aman</span>
          </div>
        </div>

        {/* ── BOTTOM: Links + Contact ──────────────────────── */}
        <div className="grid gap-10 pt-10 grid-cols-1 sm:grid-cols-3 text-center sm:text-left">
          <FooterColumn
            title="Jualakun.id"
            links={[
              { href: '/tentang', label: 'Tentang Kami' },
              { href: '/syarat-ketentuan', label: 'Syarat & Ketentuan' },
              { href: '/kebijakan-privasi', label: 'Kebijakan Privasi' },
            ]}
          />

          <FooterColumn
            title="Produk"
            links={[
              { href: '/#produk', label: 'Lihat Katalog' },
              { href: '/#cara-pesan', label: 'Cara Pesan' },
              { href: '/#faq', label: 'FAQ' },
            ]}
          />

          {/* Contact */}
          <div>
            <h4 className="font-heading text-base font-bold text-white mb-4">Hubungi Kami</h4>
            <ul className="space-y-3 text-[15px] text-white/80">
              <li className="flex items-center gap-2.5 justify-center sm:justify-start">
                <Mail size={16} className="text-brand-400 shrink-0" />
                <a href="mailto:cs@jualakun.id" className="hover:text-brand-400 transition-colors">
                  cs@jualakun.id
                </a>
              </li>
              <li className="flex items-center gap-2.5 justify-center sm:justify-start">
                <Phone size={16} className="text-brand-400 shrink-0" />
                <span>WhatsApp Support</span>
              </li>
              <li className="flex items-center gap-2.5 justify-center sm:justify-start">
                <AlertCircle size={16} className="text-brand-400 shrink-0" />
                <Link href="/kontak" className="hover:text-brand-400 transition-colors">
                  Laporan Kendala
                </Link>
              </li>
              <li className="flex items-start gap-2.5 justify-center sm:justify-start">
                <Clock size={16} className="text-brand-400 shrink-0 mt-0.5" />
                <span>Setiap hari, 06.00 – 00.00 WIB</span>
              </li>
            </ul>
          </div>
        </div>
      </div>

      {/* ── Copyright bar ─────────────────────────────────── */}
      <div className="border-t border-white/10 py-4 px-4 text-center text-sm text-white/65 font-medium">
        © {new Date().getFullYear()} Jualakun.id. Hak cipta dilindungi.
      </div>
    </footer>
  )
}

function FooterColumn({ title, links }: { title: string; links: { href: string; label: string }[] }) {
  return (
    <div>
      <h4 className="font-heading text-base font-bold text-white mb-4">{title}</h4>
      <ul className="space-y-2.5 text-[15px] text-white/80">
        {links.map((link) => (
          <li key={link.href}>
            <Link href={link.href} className="hover:text-brand-400 transition-colors">
              {link.label}
            </Link>
          </li>
        ))}
      </ul>
    </div>
  )
}
